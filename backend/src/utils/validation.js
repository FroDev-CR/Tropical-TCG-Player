...
// Utilidades de validación para Costa Rica (migrado desde frontend)
...
/**
 * Valida provincia de Costa Rica
 */
const validateProvince = (province) => {
  const validProvinces = ['San José', 'Alajuela', 'Cartago', 'Heredia', 'Guanacaste', 'Puntarenas', 'Limón'];
  
  if (!province) return { valid: false, error: 'Provincia es requerida' };
  
  if (!validProvinces.includes(province)) {
    return { valid: false, error: 'Provincia no válida para Costa Rica' };
  }
...
const validateRegistrationData = (userData) => {
  ...
  const phoneValidation = validatePhoneFormat(userData.phone);
  if (!phoneValidation.valid) errors.phone = phoneValidation.error;
  
  const cedulaValidation = validateCedulaFormat(userData.cedula);
  if (!cedulaValidation.valid) errors.cedula = cedulaValidation.error;
  
  const provinceValidation = validateProvince(userData.province);
  ...
  return {
    ...
    formattedData: isValid ? {
      ...
      cedula: cedulaValidation.formatted || userData.cedula,
