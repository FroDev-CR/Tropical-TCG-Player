...
export const migrateUsers = functions.https.onCall(async (data, context) => {
...
export const migrateListings = functions.https.onCall(async (data, context) => {
  if (!context.auth) {
    throw new functions.https.HttpsError('unauthenticated', 'Usuario no autenticado');
  }
...
export const migrateTransactions = functions.https.onCall(async (data, context) => {
  if (!context.auth) {
    throw new functions.https.HttpsError('unauthenticated', 'Usuario no autenticado');
  }
  ...
  try {
    ...
    for (const transactionDoc of transactionsSnapshot.docs) {
      ...
      const newTransactionData = {
        ...
        sellerId: oldData.sellerId || (oldData.items?.[0]?.sellerId) || '',
        sellerName: oldData.sellerName || (oldData.items?.[0]?.sellerName) || '',
...
function convertOldStatus(oldStatus: string): string {
...
export const checkMigrationStatus = functions.https.onCall(async (data, context) => {
  if (!context.auth) {
    throw new functions.https.HttpsError('unauthenticated', 'Usuario no autenticado');
  }
  ...
  try {
    ...
    const usersMigrated = usersSnapshot.docs.every(doc => 
      doc.data().verificationStatus && doc.data().suspensionStatus
    );
    
    // Verificar listings
    const listingsSnapshot = await db.collection('listings').limit(5).get();
    const listingsMigrated = listingsSnapshot.docs.every(doc => 
      doc.data().reservedQuantity !== undefined
    );
    
    // Verificar transactions
    const transactionsSnapshot = await db.collection('transactions').limit(5).get();
    const transactionsMigrated = transactionsSnapshot.docs.every(doc => 
      doc.data().timeline && doc.data().deliveryInfo
    );
...
export { seedStores, createEmptyCollections, verifyCollections } from './seedData';